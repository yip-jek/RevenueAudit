...
void BaseJHive::Init(const std::string& load_jar_path) throw(Exception)
{
	...
	if ( m_pJNI != NULL )
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] Already initialized! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}
	...
	if ( NULL == m_pJNI )
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] new StructJNI failed: Allocate memory failed ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}
...
void BaseJHive::Connect() throw(Exception)
{
	if ( NULL == m_pJNI )
	{
		throw Exception(BJH_CONNECT_FAILED, "[HIVE] NOT initialized ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}
	...
	{
	case 0:
		m_pLog->Output("[HIVE] Connect <%s:%d> OK.", m_strHost.c_str(), m_nPort);
		break;
	case 1:
	case -1:
	case -2:
		throw Exception(BJH_CONNECT_FAILED, "[HIVE] Connect failed: %s [FILE:%s, LINE:%d]", GetErrorMsg().c_str(), __FILE__, __LINE__);
		break;
	default:
		throw Exception(BJH_CONNECT_FAILED, "[HIVE] Connect failed: Unknown error - %d [FILE:%s, LINE:%d]", res_conn, __FILE__, __LINE__);
		break;
	}
}
...
bool BaseJHive::Disconnect()
{
	if ( m_pJNI != NULL )
	{
		...
		{
			m_pLog->Output("[HIVE] Disconnect OK.");
			return true;
		}
		else
		{
			m_pLog->Output("[HIVE] Disconnect failed: %s", GetErrorMsg().c_str());
			return false;
		}
	}
...
void BaseJHive::ExecuteSQL(const std::string& sql) throw(Exception)
{
	if ( NULL == m_pJNI )
	{
		throw Exception(BJH_EXECUTE_SQL_FAILED, "[HIVE] NOT initialized ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	m_pLog->Output("[HIVE] Execute SQL: %s", sql.c_str());
	...
	{
		throw Exception(BJH_EXECUTE_SQL_FAILED, "[HIVE] Execute SQL failed: %s [FILE:%s, LINE:%d]", GetErrorMsg().c_str(), __FILE__, __LINE__);
	}
}
...
void BaseJHive::FetchSQL(const std::string& sql, std::vector<std::vector<std::string> >& vec2_data) throw(Exception)
{
	if ( NULL == m_pJNI )
	{
		throw Exception(BJH_FETCH_DATA_FAILED, "[HIVE] NOT initialized ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	m_pLog->Output("[HIVE] Fetch SQL: %s", sql.c_str());
	...
	{
		throw Exception(BJH_FETCH_DATA_FAILED, "[HIVE] Fetch data failed: ArrayList is null ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	const jint ARRAY_SIZE = m_pJNI->p_jni_env->CallIntMethod(jobj_arraylist, m_pJNI->jmid_array_size);
	m_pLog->Output("[HIVE] Fetch data size: %ld", ARRAY_SIZE);

	if ( ARRAY_SIZE <= 0 )
	{
		throw Exception(BJH_FETCH_DATA_FAILED, "[HIVE] Fetch data failed: %s [FILE:%s, LINE:%d]", GetErrorMsg().c_str(), __FILE__, __LINE__);
	}
	...
	m_pLog->Output("[HIVE] Fetch data OK.");
}
...
std::string BaseJHive::GetJarClasspath(const std::string& load_jar_path) throw(Exception)
{
	...
	{
		throw Exception(BJH_GET_CLASSPATH_FAILED, "[HIVE] The jar load_path is not set correctly! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	std::string str;
	if ( !dir_jar.Open(&str) )
	{
		throw Exception(BJH_GET_CLASSPATH_FAILED, "[HIVE] Open jar load_path \"%s\" failed: %s ! [FILE:%s, LINE:%d]", load_jar_path.c_str(), str.c_str(), __FILE__, __LINE__);
	}
	...
	if ( str_classpath.empty() )
	{
		throw Exception(BJH_GET_CLASSPATH_FAILED, "[HIVE] NO jar package found ! [FILE:%s, LINE:%d]", load_jar_path.c_str(), str.c_str(), __FILE__, __LINE__);
	}
...
void BaseJHive::CreateJVM(const std::string& load_jar_path) throw(Exception)
{
	...
	if ( NULL == pstr_op )
	{
		throw Exception(BJH_CREATE_JVM_FAILED, "[HIVE] new char[] failed: Allocate memory failed ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}
	...
	m_pLog->Output("[HIVE] JavaVMInitArgs.version            = JNI_VERSION_1_6");
	m_pLog->Output("[HIVE] JavaVMInitArgs.nOptions           = 1");
	...
	if ( res < 0 )
	{
		throw Exception(BJH_CREATE_JVM_FAILED, "[HIVE] Create JavaVM failed: %d [FILE:%s, LINE:%d]", res, __FILE__, __LINE__);
	}

	m_pLog->Output("[HIVE] Create JavaVM OK.");
}
...
void BaseJHive::DestroyJVM()
{
	if ( m_pJNI->p_jvm != NULL )
	{
		...
		m_pLog->Output("[HIVE] Destroy JavaVM OK.");
	}
}
...
void BaseJHive::InitHiveAgent() throw(Exception)
{
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] Can not find Java class: %s [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	if ( NULL == mid )
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO construct method of %s ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] Construct %s failed ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO disconnect method of %s ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO isconnected method of %s ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO geterrormsg method of %s ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO executesql method of %s ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO fetchdata method of %s ! [FILE:%s, LINE:%d]", JAVA_HIVE_CLASS_NAME.c_str(), __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] Can not find Java class: java/util/ArrayList [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO size method of ArrayList ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}
	...
	{
		throw Exception(BJH_INIT_FAILED, "[HIVE] NO get method of ArrayList ! [FILE:%s, LINE:%d]", __FILE__, __LINE__);
	}

	m_pLog->Output("[HIVE] Init %s OK.", JAVA_HIVE_CLASS_NAME.c_str());
}
